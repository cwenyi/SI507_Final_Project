...
CACHE_FILENAME = 'drama.json'
DBNAME = 'drama_imdb.db'
CACHE_DICT = {}
...
def build_movie_dict():
    ...
    for movie in tqdm(movie_list):
        ...
        top_movies[ranking] = {}
        top_movies[ranking]['movie_name'] = movie_name
        top_movies[ranking]['full_link'] = full_link
        top_movies[ranking]['ranking'] = ranking
    ...
    for movie in tqdm(top_movies):
        ...
        try:
            creator = []
            for item in Json['creator']:
                if item['@type'] == 'Person':
                    creator.append(item['name'])
            top_movies[movie]['creator'] = ', '.join([item for item in creator])
        except:
            top_movies[movie]['creator'] = ''

        try:
            top_movies[movie]['actor'] = Json['actor']['name']
        except:
            top_movies[movie]['actor'] = ', '.join([item['name'] for item in Json['actor']]) 
...
def insert_movies():
    ...
    for movie in top_movies:
        ...
        insertion = (None, top_movies[movie]['ranking'], top_movies[movie]['movie_name'], 
...
def insert_directors():
    ...
    for movie in top_movies:
        ...
        insertion = (None, top_movies[movie]['movie_name'], top_movies[movie]['creator'])
        statement = 'INSERT INTO "creators" '
        statement += 'VALUES (?, ?, ?)'
        cur.execute(statement, insertion)
...
def insert_stars():
    ...
    for movie in top_movies:
        ...
        insertion = (None, top_movies[movie]['movie_name'], top_movies[movie]['actor'])
        statement = 'INSERT INTO "actors" '
        statement += 'VALUES (?, ?, ?)'
        cur.execute(statement, insertion)
...
def plot_director_count():
    ...
    statement = '''
        WITH split(word, str) AS (
            SELECT '', DirectorName||',' FROM creators
            UNION ALL SELECT
...
def interactive_prompt():
    ...
    response = ''
    print("Find out more about top 250 TV series of all time! Enter 'help' to see the options")
